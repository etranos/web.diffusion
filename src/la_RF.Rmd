...
# Summary of sample sizes: 5055, 4690, 4732, 5070, 5040, 4718, ... 
# Resampling results across tuning parameters:
# 
#   mtry  splitrule   RMSE        Rsquared   MAE       
#   2     variance    0.03227897  0.8023185  0.01991724
#   2     extratrees  0.03150004  0.8057960  0.01912348
#   4     variance    0.03271887  0.7985855  0.02025685
#   4     extratrees  0.03174791  0.8047090  0.01932473
#   6     variance    0.03358260  0.7879953  0.02080616
#   6     extratrees  0.03207532  0.8018495  0.01964516
# 
...
varimp_mars$importance <- varimp_mars$importance %>% 
  ...
  mutate(rowname = ifelse(rowname == "dist", "distance to the nearest city",
                          ifelse(rowname == "n.London.lag", "London's web density, t-1", 
                                 ifelse(rowname == "London", "distance to London",
                                        ifelse(rowname == "n.l.slag", "spatial and temporal lag of web density",
                                                             ifelse(rowname == "n.nearest.city.lag", "nearest city's web density, t-1", rowname)))))) %>% 
  column_to_rownames()
...
path.out <- paste0(path, "/outputs/rf/figures/test_regions.csv")
rf.year.all.metrics %>% 
  as.data.frame() %>%
