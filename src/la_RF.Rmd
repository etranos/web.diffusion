...
df <- df %>% drop_na() %>% # THIS DROPS L999999
  group_by(year) %>% 
...
for (t in years){
  ...
  model.all <- train(n ~ area + dist + dist.retail + London + south + #growth ~ 
                     ...
                     #%>% mutate(growth = ifelse(growth== -Inf | growth == Inf, NA, growth))
                     trControl = tc,
                     method = "ranger", #"rpart", # rf lm
                     na.action = na.omit,
                     #preProc = c("center", "scale"),
                     importance = 'impurity') # TRUE for rf
...
df <- df %>% left_join(df %>% ungroup() %>% filter(LAD21NM == "Islington") %>% dplyr::select(year, n) %>% rename(n.London = n)) %>% 
  group_by(ladcd) %>% 
...
```{r}
set.seed(123)
indices <- CreateSpacetimeFolds(df %>% filter(year > 1999), 
                                spacevar = "ladcd",
                                timevar = "year", 
                                k = 10)
...
start.time <- Sys.time()
model.all <- train(growth ~ #area + dist + dist.retail + London + south + n.slag + n.lag + n.l.slag + London.n.lag + n.nearest.city + year,
                     I(n.London.lag/dist) + I(n.nearest.city.lag/London) + n.l.slag + year,
                   data = df %>% filter(year > 1999), 
                   trControl = tc,
                   method = "ranger",
                   na.action = na.omit,
                   #preProc = c("center", "scale"),
...
# Summary of sample sizes: 4718, 4690, 5040, 4718, 5055, 4704, ... 
# Resampling results across tuning parameters:
# 
#   mtry  splitrule   RMSE        Rsquared   MAE       
#    2    variance    0.02194750  0.9015001  0.01162254
#    2    extratrees  0.02245492  0.8984940  0.01229071
#    6    variance    0.02208770  0.9025087  0.01175267
#    6    extratrees  0.02165214  0.9081808  0.01161281
#   10    variance    0.02447416  0.8859265  0.01326348
#   10    extratrees  0.02204464  0.9064730  0.01184945
# 
# Tuning parameter 'min.node.size' was held constant at a value of 5
# RMSE was used to select the optimal model using the smallest value.
# The final values used for the model were mtry = 6, splitrule = extratrees and min.node.size = 5.
...
path.out <- paste0(path, "/outputs/rf/varimp_LA.png")
...
for (i in regions){
  ...
  set.seed(71)
  indices <- CreateSpacetimeFolds(df %>% filter(RGN21CD!=i), 
                                spacevar = "ladcd",
                                timevar = "year", 
                                k = 10)
  ...
  model.all <- train(n ~ #area + dist + dist.retail + London + south + n.slag + n.lag + n.l.slag + n.nearest.city + year,
                       I(n.London.lag/dist) + I(n.nearest.city.lag/London) + n.l.slag + year,
                   data = df %>% filter(RGN21CD!=i), 
                   trControl = tc,
                   method = "ranger",
                   na.action = na.omit,
                   #preProc = c("center", "scale"),
                   importance = "impurity")
  assign(paste0("model.all",as.character(i)), model.all) # name = region not in the training data
}

stopCluster(cl)
end.time <- Sys.time() # ca. 2h
time.taken <- round(end.time - start.time,2)
time.taken
```
...
df.predictions <- data.frame()
for (i in regions){
    pred <- predict(fit.model.all[names(fit.model.all)==paste0("model.all",i)], df[df$RGN21CD==i,])
    pred <- as.data.frame(pred)
    rownames(pred) <- c()
    assign(paste0("region.predict.model.", ".on.", i), pred)
    
    d <- paste0("region.predict.model.", ".on.", i)
    d <- cbind(get(d), df %>% filter(RGN21CD==i)) #%>% arrange(year, ladcd)
    d <- d[,c(5, 2, 4, 6, 1)]
    colnames(d)[5] <- "predictions"
...
path.out <- paste0(path, "/outputs/rf/heatmap_LA.png")
rf.year.all.metrics %>% 
  as.data.frame() %>%
