...
```{r}
df.test <- df %>% filter(oa11cd == "S00103828") 
...
for (oa in all.oa){
  ...
  tryCatch(b <- tidy(nls(n ~ SSlogis(year, Asym, xmid, scal), data = data)), error=function(e) NULL)
...
```{r}
path.out <- paste0(path, "/data/temp/s.csv")
...
oa.gb.f <- oa.gb.f %>% left_join(a, by = "id") %>% 
...
#   arrange(order) %>% 
ggplot(data = oa.gb.f %>% arrange(order), aes(x = long, y = lat)) +
...
oa.class.df <- a %>%  
  mutate(fast = ifelse(estimate < 2003, "fast", "slow")) %>% 
  left_join(oa.class@data, by = c("id"="OA_SA")) %>% 
  select(1:10) %>% 
  arrange(id) 
...
oa.class.df %>% 
  ...
  left_join(pen %>% select(code, name),
            by = c("SUBGRP" = "code")) %>% 
  kable()
...
oa.class.df <- oa.class.df %>% left_join(dist, by = c("id" = "oa11cd")) 
...
oa.class.df <- oa.class.df %>% left_join(dist.retail, by = c("id" = "oa11cd")) 
