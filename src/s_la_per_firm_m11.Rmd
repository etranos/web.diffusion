...
path.out <- paste0(path, "/outputs/s_uk_per_firm_11.png")
...
a.la <- a.la %>% filter(term=="xmid") %>% 
  ...
  mutate(fast = ifelse(estimate < 2002, "fast", "slow")) 
...
# plots
path.out <- paste0(path, "/outputs/s_la_per_firm_m11/lad11_")

for (i in all.la){
  
  xmid_la <- pred %>% left_join(a.la) %>% filter(ladcd == i) %>% dplyr::select(estimate) %>% first()
  lines <- data.frame(
    intercepts = c(2002, xmid_la[1]),
    Xmid = c("UK", i)
  )
...
```{r eval=F}
path.out <- paste0(path, "/data/temp/s_la_per_firm_11.csv")
