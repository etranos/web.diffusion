...
path.out <- paste0(path, "/outputs/lisa_level_pc", n, "_la") # for unique PC
#path.out <- paste0(path, "/outputs/lisa_level_pc", m, "_la")  # for multiple PC
...
```{r}
path.out <- paste0(path, "/outputs/morani_la.csv")
morani %>% as_tibble() %>% 
  rename('morani'=V1,
                   p = V2,
                   year = V3) %>% 
