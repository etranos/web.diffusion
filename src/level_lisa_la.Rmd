...
set.seed(71)
m <- train(n ~ density + area + dist + dist.retail + London +
...
```{r}
hepl <- df %>% createTimeSlices(initialWindow = 5,
                        horizon = 1,
                        fixedWindow = F)
...
set.seed(71)
m.test <- train(n ~ area + dist + dist.retail + London + #density
             n.slag + year + n.nearest.city + n.l.slag,
           data = df %>% filter(year < 1998),
           trControl = tc,
           method = "rf", 
           importance = TRUE,
           na.action = na.omit)
...
pred <- predict(m.test, df %>% ungroup() %>% filter(year==1998), na.action = na.pass)
length(pred)

pred <- cbind(pred %>% as_tibble(), df %>% ungroup() %>% filter(year==1998))
postResample(pred = pred$value, obs = pred$n) ## 0.99
```
