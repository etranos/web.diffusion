...
path.out <- paste0(path, "/outputs/lisa_level_pc", n, "_oa") # for unique PC
#path.out <- paste0(path, "/outputs/lisa_level_pc", m, "_oa")  # for multiple PC
...
```{r}
path.out <- paste0(path, "/outputs/morani.csv")
morani %>% as_tibble() %>% 
  rename('morani'=V1,
                   p = V2,
                   year = V3) %>% 
...
path.out <- paste0(path, "/outputs/gini.csv")
...
path.out <- paste0(path,"/outputs/rsq_dist_london.png")
ggplot(dens.London) +
  geom_line(aes(x=year, y = rsquared)) +
  ylab("Rsquared") + 
  ggtitle("Regression fit, distance to London on website density") +
...
path.out <- paste0(path,"/outputs/b_dist_london.png")
...
#ggplot(out.model, aes(x=year, y = estimate)) +  
  ...
  ggtitle("Regression coefficients, distance to London on website density") +
  theme(plot.title = element_text(hjust = 0.5)) +
  ggsave(path.out)
```
...
path.out <- paste0(path,"/outputs/rsq_dist_city.png")
ggplot(dens.distMet) +
  geom_line(aes(x=year, y = rsquared)) +
  ylab("Rsquared") + 
  ggtitle("Regression fit, distance to nearest city on website density") +
  theme(plot.title = element_text(hjust = 0.5)) +
  ggsave(path.out)
...
path.out <- paste0(path,"/outputs/b_dist_city.png")
ggplot(out.modelMet, aes(x=reorder(year, year), y=estimate)) +
  ...
  geom_point() + coord_flip() +
  ylab("distance coefficient") + xlab("") + 
  ggtitle("Regression coefficients, distance to nearest city on website density") +
  theme(plot.title = element_text(hjust = 0.5)) +
  ggsave(path.out)
```
...
path.out <- paste0(path,"/outputs/rsq_dist_centre.png")
ggplot(dens.dist.retail) +
  geom_line(aes(x=year, y = rsquared)) +
  ylab("Rsquared") + 
  ggtitle("Regression fit, distance to nearest retail centre on website density") +
  theme(plot.title = element_text(hjust = 0.5)) +
  ggsave(path.out)
...
path.out <- paste0(path,"/outputs/b_dist_centre.png")
