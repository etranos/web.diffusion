...
```{r}
path.out <- paste0(path, "/outputs/morani_oa.csv")
morani %>% as_tibble() %>% 
  rename('morani'=V1,
                   p = V2,
                   year = V3) %>% 
...
path.out <- paste0(path, "/outputs/gini_oa.csv")
...
path.out <- paste0(path,"/outputs/rsq_dist_london_oa.png")
ggplot(dens.London) +
...
path.out <- paste0(path,"/outputs/b_dist_london_oa.png")
...
path.out <- paste0(path,"/outputs/rsq_dist_city_oa.png")
ggplot(dens.distMet) +
  geom_line(aes(x=year, y = rsquared)) +
  ylab("Rsquared") + 
...
path.out <- paste0(path,"/outputs/b_dist_city_oa.png")
...
path.out <- paste0(path,"/outputs/rsq_dist_centre_oa.png")
...
path.out <- paste0(path,"/outputs/b_dist_centre_oa.png")
...
```{r}
path.out <- paste0(path, "/outputs/r2_all_oa.csv")
