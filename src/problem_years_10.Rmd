...
path.out <- paste0(path, "/outputs/pc_year_10.png")
...
path.out <- paste0(path, "/outputs/pc_year_10_corrected.png")
ggsave(path.out, bg='#ffffff')  
```
...
```{r}
path.out <- paste0(path, "/outputs/lisa/corrected/LA/morani_la_10.csv")
morani %>% as_tibble() %>% 
  rename('morani'=V1,
                   p = V2,
                   year = V3) %>% 
...
path.out <- paste0(path, "/outputs/gini/gini_la_10.csv")
...
```{r}
path.out <- paste0(path, "/outputs/lisa/corrected/OA/morani_oa_10.csv")
morani %>% as_tibble() %>% 
  rename('morani'=V1,
                   p = V2,
                   year = V3) %>% 
...
path.out <- paste0(path, "/outputs/gini/gini_oa_10.csv")
gini.df %>% write_csv(path.out)
```
