...
out.path <- paste0(path, "/outputs/ranks/web11_per_firm", t,"_2012_only95.png")
...
df %>% group_by(year) %>% 
  ...
  # t= 1996: q.95 = 243.625, q.9 = 191.5, q.1 = 11.5, q.05 = 6. **abs()**

  mutate(alpha = ifelse(abs(diff_1st) > 243.625, .8, 0.1)) %>% 
...
```{r}
out.path <- paste0(path, "/outputs/ranks/web_2000_2012_only9.png")
...
df %>% group_by(year) %>% 
  ...
  mutate(alpha = ifelse(abs(diff_1st) > 71, .8, 0.1)) %>% 
  mutate(alpha = ifelse(is.na(diff_1st), 0.1, alpha)) %>%
  mutate(label = ifelse(alpha==.1, "", ladnm)) %>% 
...
```{r}
out.path <- paste0(path, "/outputs/ranks/web11_per_year.png")
...
```{r}  
out.path <- paste0(path, "/outputs/ranks/web11_per_firm1996_2012_95.png")
