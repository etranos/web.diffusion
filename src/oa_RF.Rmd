...
oa.gb.c <- st_centroid(oa.gb) %>%
  ...
  as.data.frame() %>%
  #add_rownames(var = "id") %>% 
  bind_cols(oa.gb %>% dplyr::select(id))
...
# calculate area
oa.gb$area <- st_area(oa.gb) #/ 1000000

# Join with distance and area
df <- df %>% left_join(dist, by = c("id" = "oa11cd")) %>% 
  rename(oa11cd=id) %>% 
  left_join(oa.gb, by = c("oa11cd" = "id")) %>% 
...
dist.retail <- dist.retail %>% mutate(dist.retail.name = names(.)[max.col(.[2:6424]*-1)+1L])
...
# Minimum distance 
dist.retail <- transform(dist.retail, dist.retail = do.call(pmin, dist.retail[2:6424])) 

dist.retail <- dist.retail  %>% dplyr::select(oa11cd, dist.retail.name, dist.retail)
# 232,296 
#sapply(dist.retail, function(x) sum(is.na(x))) #0

#spatial join retail centroids into OA
retail.major.cetres.help <- st_centroid(retail) %>% 
  st_transform(4326)

# lookup obj with retail id and OA id
help <- st_join(retail.major.cetres.help, oa.gb) %>% 
  as_tibble %>% 
  mutate(dist.retail.name = row_number(),
         dist.retail.name = paste0("V", dist.retail.name)) %>% 
  dplyr::select(id, dist.retail.name) %>% 
  rename(oa11cd.retail = id)

dist.retail <- dist.retail %>% 
  left_join(help, by = c('dist.retail.name' = 'dist.retail.name')) %>% 
  dplyr::select(-dist.retail.name)
sapply(dist.retail, function(x) sum(is.na(x))) 
# 1 retail centre is missing, because the centroid falls in the water

dist.retail %>% filter(is.na(oa11cd.retail))
missing.retail.centre <- retail.major.cetres.help %>% filter(RC_ID=="RC_EW_24")
st_nearest_feature(missing.retail.centre, oa.gb)
oa.gb[91986,] # missing OA: E00094765 

dist.retail <- dist.retail %>%
  mutate(oa11cd.retail = ifelse(is.na(oa11cd.retail), "E00094765", oa11cd.retail))

# dist.retail %>% distinct(dist.retail.name) #6405
# help %>% distinct(dist.retail.name) # 6423 
# dist.retail %>% distinct(oa11cd.retail) #6217 as there are multiple centres in an OA

# Join with complete panel
df <- df %>% left_join(dist.retail, by = c("oa11cd" = "oa11cd")) 

sapply(df, function(x) sum(is.na(x)))
df %>% filter(is.na(oa11cd.retail)) %>% distinct(oa11cd) #493 rows or 29 OA don't have oa11cd.retail

# df <- df %>% dplyr::select(-dist.retail.x, -oa11cd.retail.x) %>% 
#   rename(dist.retail = dist.retail.y,
#          oa11cd.retail = oa11cd.retail.y)
```
...
sf_use_s2(FALSE)
help1 <- st_filter( oa.gb, cities.sf) %>% as_tibble %>% dplyr::select(id) %>% 
...
# NEED TO ADD NI. NOT A PROBLEM FOR NOW AS IT IS FIXED IN THE TEST SECTION
...
# regional identifier
df <- df %>% mutate(help = substr(oa11cd, start = 1, stop = 2)) %>% 
...
df <- df %>% filter(year > 1996) %>% 
  dplyr::select(oa11cd, year, RGN11CD, n, n.l.slag, n.nearest.city.lag, n.nearest.retail.lag, n.London.lag, London, dist, dist.retail) #growth, abs.growth, RGN11NM, 
...
# 
# df %>% dplyr::select(n.London.lag, n.nearest.city.lag, n.nearest.retail.lag, n.l.slag, year, London, dist, dist.retail) %>% 
#   #sapply(function(x) sum(is.na(x)))
#   summarise(across(where(is.numeric), .fns = 
#                      list(#n = is.na,
#                           min = min,
...
for(i in 1:10) seeds[[i]]<- sample.int(n=1000, 10) # 9 after adding South. It should be RHS variables + 2
  ...
  mutate(rowname = ifelse(rowname == "dist", "distance to the nearest city",
                          ifelse(rowname == "dist.retail", "distance to the nearest retail centre", 
                                 ifelse(rowname == "London", "distance to London",
                                        ifelse(rowname == "n.slag", "spatial lag of web density",
                                               ifelse(rowname == "n.lag", "temporal lag of web density",
                                                      ifelse(rowname == "n.l.slag", "spatial and temporal lag of web density",
                                                             ifelse(rowname == "n.nearest.city", "web density in nearest city", rowname)))))))) %>% 
  column_to_rownames()

path.out <- paste0(path, "/outputs/rf/varimp_OA.png")
ggplot(varimp_mars) + theme_minimal() + labs(x="") #+ theme(panel.background = element_rect(fill = "white"))
ggsave(path.out)

path.out <- paste0(path, "/data/temp/oa_rf/all_OAs/all_OAs.RData")
...
### train the model in a loop per region
...
#(8 is the number of tuning parameter, mtry for rf, here equal to ncol(iris)-2)
for(i in 1:10) seeds[[i]]<- sample.int(n=1000, 11) # 9 after adding South
...
for (i in regions){
  ...
  model.all <- train(n ~ area + dist + dist.retail + London + south + 
                     n.slag + n.lag + n.l.slag + n.nearest.city + year,
  ...
  file <- paste0(path, "/data/temp/oa_rf/regions/region_", as.character(i), ".RData")
...
filenames <- list.files(paste0(path, "/data/temp/oa_rf/regions"), pattern = "*.RData", full.names = T)

for (i in filenames){
  load(file = i)
  i <- gsub("/home/nw19521/projects/web.diffusion/data/temp/oa_rf/regions/|.RData", "", i)
  assign(i, model.all)  
}
