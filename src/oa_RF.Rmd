...
city.boundaries.sc <- st_read(path.geo) %>% 
  st_transform(4326) %>% 
...
# merge
city.boundaries <- rbind(city.boundaries.ew, city.boundaries.sc)
...
#retail.names <- retail.major.cetres$RC_ID
names(dist.retail) <- retail.major.cetres$RC_ID #city.names 
...
sf_use_s2(FALSE)
city.boundaries.oa <- st_join(oa.gb, city.boundaries, join = st_intersects) %>% 
  as_tibble() %>% 
  filter(!is.na(name)) %>% 
...
retail.oa <- st_join(oa.gb, retail, join = st_intersects) %>% 
  as_tibble() %>% 
  filter(!is.na(RC_ID)) %>% 
  dplyr::select(id, RC_ID)

# st_join(oa.gb, retail, join = st_is_within_distance, dist = 500) %>% 
#   as_tibble() %>% 
#   filter(!is.na(RC_ID)) %>% 
#   dplyr::select(id, RC_ID)
# 
# st_filter(oa.gb, retail,  .pred = st_is_within_distance, dist = 100) %>% 
#   as_tibble() %>% 
#   filter(!is.na(RC_ID)) %>% 
#   dplyr::select(id, RC_ID)

# h <- oa.gb %>% st_intersects(retail)
...
retail.help <- df %>% inner_join(retail.oa, by = c("oa11cd" = "id")) %>% 
  group_by(RC_ID, year) %>% 
...
#sapply(df_, function(x) sum(is.na(x)))
```
...
path.out <- paste0(path, "/data/temp/oa_rf/all_OAs/knnl.RData")
