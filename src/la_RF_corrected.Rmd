...
indices <- CreateSpacetimeFolds(df,# %>% filter(year > 1999),                   # FILTER BY YEAR FOR GROWTH
                                spacevar = "RGN21CD",                           # was ladcd in 1st submission
                                timevar = "year", 
                                k = 10)
...
#   2     variance    0.02811777  0.8509273  0.01912432
#   2     extratrees  0.02782836  0.8508316  0.01901921
#   4     variance    0.02867344  0.8466191  0.01970102
#   4     extratrees  0.02795250  0.8509000  0.01911956
#   6     variance    0.02974784  0.8364575  0.02066007
#   6     extratrees  0.02830277  0.8482189  0.01944133
# 
...
path.out <- paste0(path, "/outputs/rf/figures/varimp_LA_corrected.png")
ggplot(varimp_mars) + theme_minimal() + labs(x="") #+ theme(panel.background = element_rect(fill = "white"))
ggsave(path.out)

path.out <- paste0(path, "/outputs/rf/figures/varimp_LA_corrected.csv")
...
path.out <- paste0(path, "/outputs/rf/figures/test_regions_LA_corrected.csv")
rf.year.all.metrics %>% 
  as.data.frame() %>%
...
# No pre-processing
# Resampling: Cross-Validated (10 fold) 
# Summary of sample sizes: 4044, 3685, 4032, 4044, 4044, 3696, ... 
# Resampling results across tuning parameters:
# 
#   mtry  splitrule   RMSE       Rsquared   MAE      
#   2     variance    0.2658416  0.5284505  0.1934243
#   2     extratrees  0.2612133  0.5292689  0.1900297
#   4     variance    0.2621564  0.5194092  0.1912807
#   4     extratrees  0.2611575  0.5183202  0.1910029
#   6     variance    0.2609983  0.5140347  0.1891971
#   6     extratrees  0.2607463  0.5124544  0.1907650
# 
# Tuning parameter 'min.node.size' was held constant at a value of 5
# RMSE was used to select the optimal model using the smallest value.
...
path.out <- paste0(path, "/outputs/rf/figures/varimp_LA_growth_corrected.png")
ggplot(varimp_mars) + theme_minimal() + labs(x="") #+ theme(panel.background = element_rect(fill = "white"))
ggsave(path.out)
```
...
path.out <- paste0(path, "/outputs/rf/figures/test_regions_LA_growth_corrected.csv")
rf.year.all.metrics %>% 
  as.data.frame() %>%
  rownames_to_column(var = "metrics") %>% 
  pivot_longer(!metrics, names_to = "test.on") %>% 
...
path.out <- paste0(path, "/outputs/rf/correlations/corr_LAD_corrected.csv")
r %>% as_tibble(rownames = "rowname") %>% 
  write_csv(path.out)

path.out <- paste0(path, "/outputs/rf/correlations/corr_LAD_corrected_p.csv")
