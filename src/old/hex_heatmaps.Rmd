...
lookup <- read_csv(path.lookup) %>% 
  dplyr::select(pcds, msoa11cd, dointr, doterm) %>% 
...
lookup <- read_csv(path.lookup) %>% 
  dplyr::select(pcds, oseast1m, osnrth1m) %>% 
  dplyr::rename(pc = pcds)
...
# geo_sf solution. same as lisa, but squeese hexagons
# gb %>% st_as_sf(crs = 27700) %>% 
# ggplot() + 
#     geom_sf(fill=NA, color = "black", size = 0.1, alpha = 0.8) + #geom_point(data = help.df, aes(x=X,y=Y))
#     stat_binhex(data = help.df, aes(x=X,y=Y),bins=150) + 
#     # geom_polygon(data = gb.df, aes(x=long, y = lat, group = group), #gb.df
#     #              fill=NA, color = "black", size = 0.1, alpha = 0.8)+
#     
#     #coord_equal() +
...
#     ggtitle(t)
```
## hex heatmaps log transformed

```{r}
# old code: counts per hexagon based on xy
#xy point

path.out <- paste0(path, "/outputs/hex_log", n)
...
for (t in 1996:2012){
  p <- df %>%
    ...
    #stat_binhex(bins=150) +
    stat_binhex(aes(fill=log(..count..)), bins=150) +
    geom_polygon(data = gb.df, aes(x=long, y = lat, group = group), #gb.df
                 fill=NA, color = "black", size = 0.1, alpha = 0.8)+
    scale_fill_gradientn(colours=c('light gray','blue'),
                         name='log(Frequency)',na.value=NA) + #, trans = "log10") +
    coord_equal() +
    labs(x=NULL, y=NULL) +
    geom_point(data = cities, aes(x=long, y=lat), colour = "black", size = .1) + 
    geom_text(size = 2, check_overlap = F, nudge_y = .2, data = cities, aes(x=long, y=lat, label = name)) +
