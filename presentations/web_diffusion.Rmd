---
title: 'A multi-scalar story of the diffusion of a new technology: the web'
...
path.in <- paste0(path, "/outputs/lisa/oa_ggplot/morani_la.csv")
...
path.images <- paste0(path, "/outputs/oa_ggplot/lisa/")
...
<small>
*No retail centre variables for the Local Authority models
</small>
...
```{r similarity_LA, eval = T, message=FALSE}
path.table <- paste0(path, "/outputs/rf/figures/test_regions_LA.csv")
read.csv(path.table) %>% 
...
```{r similarity_OA, out.height= "70%", out.width="70%", message=FALSE}
path.table <- paste0(path, "/outputs/rf/figures/test_regions_OA.csv")
read.csv(path.table) %>% 
  mutate(Rsquared = round(Rsquared, digits= 3)) %>% 
  kable()
```
