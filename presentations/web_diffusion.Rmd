...
**Neighborhood effect**: diffusion proceeds outwards from innovation
...
```{r, eval=FALSE, moransi_oa, results='asis'}
...
```{r lisa_oa, eval=FALSE,results='asis', out.width = "30%"}
path.images <- paste0(path, "/outputs/")
...
-   $Website\,Density_{i}$: Websites per firm in a Local Authority $i$
:::
...
```{r b_London_oa, eval=F, fig.height=5, fig.width=5, message=FALSE}
#, out.width = "60%"
...
```{r b_city_oa, eval = F, fig.height=5, fig.width=5, message=FALSE}
#, out.width = "60%"
...
```{r b_centre_oa, eval = F, message=FALSE, out.height= "60%", out.width="60%"}
...
read.csv(path.in2) %>% #left_join(read.csv(path.in2)) %>% 
  ggplot(aes(x=year)) +
...
-   Distance effect: urban gravitation increases over time and then
    drops
